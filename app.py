...
/* Consistent page width and breathing room — a wide, edge-to-edge
   layout reads as unfinished; a comfortable max-width with real top/
   bottom padding is one of the simplest things that makes an app feel
   like a finished product rather than a default framework page. */
.block-container {
    max-width: 1400px;
    padding-top: 2rem;
    padding-bottom: 3rem;
...
/* A slightly distinct sidebar background — subtle separation from the
   main content instead of the two blending into one flat surface. */
[data-testid="stSidebar"] {
    background: #F5F3EE;
    border-right: 1px solid #E5E2DA;
}

/* Card treatment for featured metrics — the landing page highlights
   and the Scorecard specifically, NOT the dense data grids (key
   levels, fundamentals), which already needed to get SMALLER, not
   heavier, per earlier feedback. This is deliberately light, not the
   dark gradient a fintech-dark-theme reference would use — same
   structural idea (a bordered, gently-elevated card), translated into
   the app's own warm palette instead of copying dark colors over. */
.metric-card {
...
.metric-card .metric-card-delta {
    color: #6B7280;
    font-size: 11px;
    margin-top: 2px;
}

/* Rounded, slightly bolder buttons and expanders — small polish that
   adds up; default Streamlit buttons/expanders are perfectly
   functional but visually anonymous. */
.stButton > button {
    border-radius: 8px;
    font-weight: 600;
}
...
def metric_card(label: str, value: str, delta: str = None) -> str:
    """
    A bordered, gently-elevated card version of a metric — for
    FEATURED highlights only (the landing page stats, the Scorecard),
    not the dense data grids. Those grids (key_levels, the 17-metric
    fundamentals table) specifically needed to get SMALLER per earlier
    feedback about excess whitespace, so they stay on compact_metric;
    wrapping every cell in a heavy card there would recreate that exact
    problem. This is for the handful of places meant to draw the eye.
    """
...
def render_quarterly_and_fundamentals(ticker: str, currency: str = "$"):
    ...
    for col, (category, data) in zip(score_cols, scorecard.items()):
        ...
        with col:
            st.markdown(metric_card(category.capitalize(), score_display, data["label"]), unsafe_allow_html=True)
...
# re-running the 5-subagent pipeline or losing the result entirely.
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown(metric_card("Coverage", "5 factors", "fundamentals to risk"), unsafe_allow_html=True)
    with col2:
        st.markdown(metric_card("Markets", "US + India", "NSE / BSE supported"), unsafe_allow_html=True)
    with col3:
        st.markdown(metric_card("Cost to try", "Free", "no signup required"), unsafe_allow_html=True)
